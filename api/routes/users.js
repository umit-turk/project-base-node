...
router.post("/add", async (req, res) => {
  ...
  try {
    ...
    if(!body.email) throw CustomError(Enum.HTTP_CODES.BAD_REQUEST,"Validation Error!", "email field must be filled");

    if(is.not.email(body.email)) throw CustomError(Enum.HTTP_CODES.BAD_REQUEST,"Validation Error!", "email field must be email format");
    ...
    if(!body.roles || !Array.isArray(body.roles) || body.roles.length === 0) {
      throw new CustomError(Enum.HTTP_CODES.BAD_REQUEST, "Validation Error!","roles field must be array");
    }
